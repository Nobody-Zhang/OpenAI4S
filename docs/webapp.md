...
| Recovery Journal and verified recovery pipeline | Status/actions and the mutation routes `POST /frames/{id}/recovery/actions/{restore\|retry\|restart_fresh}` are implemented for the root/current branch. Each request owns an exact `recovery` execution ticket; candidate workers are published only after bootstrap, CAS/Artifact validation, replay-safety checks, and state validation. `restart_fresh` requires `{"confirm":true}`. Failures remain in the journal and preserve the prior generation. The browser renders live journal/status events, but dedicated recovery buttons are not yet exposed. |
