...
- **`openai4s/llm/` + `openai4s/llm.py`** — normalized replies, provider-native tool calls, wire assembly, and stdlib transport behind the compatible `chat()` facade. Supported wires include OpenAI-compatible Chat/Responses, Anthropic Messages, and Gemini `generateContent`.
